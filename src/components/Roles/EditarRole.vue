        ...
        <form @submit.prevent="submit()">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">Editar el rol</h5>
...
const submit = async () => {
  try {
    ...
    const response = await axiosInstance.put("/api/roles/" + id.value, {
      nombre: nombre.value,
...
// Función para abrir el modal y cargar datos del rol
const openModal = (rol) => {
  getPermisos();
  nombre.value = "";
  permisos.value = [];
...
const cargarData = (rol) => {
  id.value = rol.id;
  nombre.value = rol.nombre;
  permisos.value = rol.permissions.map((permiso) => permiso.id);
};
