            ...
            <div class="container-fluid">
                <div class="row g-2 align-items-center">
                    <div class="col"><!-- Page pre-title -->
                        ...
                        <h2 class="page-title">Actualizar Página</h2>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="row g-3 align-items-center pb-3">
                    <div class="col-md-auto ms-auto d-print-none mt-0">
                        <div class="btn-list">
                            ...
                            <router-link :to="{ name: 'PageFile', params: { slug: slug } }" class="btn btn-warning">
                                <!-- Download SVG icon from http://tabler-icons.io/i/settings -->
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round"
...
const image_banner = ref("");
const image_menu = ref("");
...
const submit = async () => {
    ...
    try {
        const response = await axiosInstance.post(`/api/pages/${id.value}`, formData, {
...
const loadData = async (page) => {
    ...
    try {
        ...
        content.value = response.data.data.content === null ? '' : response.data.data.content;
        created_at.value = response.data.data.created_at;
        created_by.value = response.data.data.created_by;
        date.value = response.data.data.date;
        deleted_by.value = response.data.data.deleted_by;
        image_banner.value = response.data.data.image_banner;
        image_menu.value = response.data.data.image_menu;
        status.value = response.data.data.status;
        title.value = response.data.data.title;
        updated_at.value = response.data.data.updated_at;
        updated_by.value = response.data.data.updated_by;
