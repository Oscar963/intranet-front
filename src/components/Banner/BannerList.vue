<template>
    <div>
        <div class="banner-header d-print-none">
            <div class="container-fluid">
                <div class="row g-2 align-items-center">
                    <div class="col"><!-- Page pre-title -->
                        <div class="banner-pretitle">Módulo</div>
                        ...
                        <div class="card">
                            <div class="card-header">
                                ...
                                <div class="card-actions">
                                    <div class="btn-list">
                                        <router-link v-if="$can('banner', 'create')"
                                ...
                                <table class="table card-table text-nowrap table-hover">
                                    ...
                                    <tbody class="align-middle">
                                        ...
                                        <tr v-else v-for="(item, index) in listadoBanners" :key="index">
                                            ...
                                            <td>
                                                <router-link v-if="$can('banner', 'edit')"
                                                    class="text-dark px-1 cursor-pointer" aria-label="Button"
                                                    data-bs-toggle="tooltip" data-bs-placement="top" title="Editar"
                                                    :to="{ name: 'BannerUpdate', params: { id: item.id } }">
                                                    <!-- Download SVG icon from http://tabler-icons.io/i/plus -->
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                        viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                                ...
                                                <a v-if="$can('banner', 'delete')" @click="deleteBanners(item)"
...
const listarDatos = async () => {
    ...
    try {
        ...
        listadoBanners.value = response.data.data;
    } catch (error) {
        console.log(error);
    } finally {
        loading.value = false;
...
const deleteBanners = async (banner) => {
    try {
        const result = await Swal.fire({
            title: "¿Estás seguro que desea eliminar la banner?",
        ...
        if (result.isConfirmed) {
            const loadingSwal = Swal.fire({
                ...
                text: 'Eliminando el banner, por favor espera...',
                allowOutsideClick: false,
            });

            // Llamar a la API para eliminar la banner
        ...
        Swal.fire({
            title: "Error",
            text: "Ocurrió un error al eliminar la banner.",
            icon: "error",
            confirmButtonColor: "#004693",
            confirmButtonText: "Cerrar",
        });
